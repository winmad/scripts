...
for i in range(len(fovs)): #[15, 8, 4, 2, 1, 0.4]:
	for j in range(len(scales)): #[2, 4]:
		for k in range(len(parts)):
			split_part = parts[k]
			ref_filename = "gabardine\\albedo_sv_search\\ref_area\\fov_" + str(fovs[i]) + "_spp_256_scale_1x_" + str(parts[k]) + ".pfm"
			...
			for iter in range(max_iter):
				...
				filename += "_spp_" + str(sample_count) + "_scale_" + str(scale_density) + "x_" + str(parts[k]) + ".pfm"
				args += filename
			...
			outfile = open("gabardine\\albedo_sv_search\\albedo_sv_values.txt", "a")
			st = "fov: " + str(fov) + "  scale: " + str(scale_phase) + " part: " + str(split_part) + "\n"
			outfile.write(st)
