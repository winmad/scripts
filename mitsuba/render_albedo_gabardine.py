...
for i in range(len(fovs)): #[15, 8, 4, 2, 1, 0.4]:
	ref_filename = "gabardine\\ref_area\\fov_" + str(fovs[i]) + "_spp_256_phase_1x_density_1x.pfm"
	...
	for j in range(len(scales)): #[2, 4]:
		...
		for iter in range(max_iter):
			...
			args += " -p 11 -o "
			filename = "gabardine\\albedo_search\\fov_" + str(fov)
			filename += "_spp_" + str(sample_count) + "_scale_" + str(scale_density) + "x.pfm"
			args += filename
			args += " gabardine\\gabardine_tiled_albedo.xml"
			print cmd + args
			os.system(cmd + args)
		...
		outfile = open("gabardine\\albedo_search\\albedo_values.txt", "a")
